$(document).ready(function () {
    ...
    window.setInterval(function(){
        if($('#statusUserLive').html() == "Liberada"){
            $("#cardProgress").load(location.href + " #cardProgress>*", "");
        }
    }, 1000);
    ...
    */
    ...
    $('#removeUser').on('show.bs.modal', function (event) {
        ...
        var idUser = button.data('idremoveusuario');
        var modal = $(this);
        modal.find('.modal-title').text('Remover Usuário');
        $('#idUserRemove').val(idUser);
    });
    
    $('#formRemoveUser').submit(function () { 	//Ao submeter formulário
        var url = "actions/removeUser.php"; //Caminho do arquivo php
        var formDados = new FormData($(this)[0]); //Pega os valores dos campos
        $.ajax({    //Função AJAX
            type: "POST", //Tipo da passagem dos dados
            url: url, // local do arquivo php
            ...
            beforeSend: function () {
                var alertBox = '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>AGUARDE</strong> estamos analizando seus dados!</div>';
                $('#resultRemoveUser').html(alertBox);
            },
            complete: function () { },
            success: function (data) {
                $('#resultRemoveUser').html(data);
                $("#userListView").load(location.href + " #userListView>*", "");
            }
        });
        return false;	//Evita que a página seja atualizada
   ...
   $('#removeAcessoAll').on('show.bs.modal', function (event) {
      var button = $(event.relatedTarget);
      var idUser = button.data('iduseracessall');
      var modal = $(this);
      modal.find('.modal-title').text('Remover Acesso');
      $('#idUserRemoveAcessAll').val(idUser);
    });
    
    $('#formRemoveAcessAll').submit(function () { 	//Ao submeter formulário
        var url = "actions/removeAcessoAll.php"; //Caminho do arquivo php
        var formDados = new FormData($(this)[0]); //Pega os valores dos campos
        $.ajax({    //Função AJAX
            type: "POST", //Tipo da passagem dos dados
            url: url, // local do arquivo php
            ...
            beforeSend: function () {
                var alertBox = '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>AGUARDE</strong> estamos analizando seus dados!</div>';
                $('#resultRemoveAcessAll').html(alertBox);
            },
            complete: function () { },
            success: function (data) {
                $('#resultRemoveAcessAll').html(data);
                $("#accessListView").load(location.href + " #accessListView>*", "");
                 $('#removeAcessoAll').modal('toggle');
            }
        });
        return false;	//Evita que a página seja atualizada
    });
    ...
    */
    ...
    var live = document.getElementById("statusUserLive").innerHTML;
    if(live == "Bloqueada"){
        $('.btnLive').attr("disabled","disabled");
    }
